...
with v_career as (
    ...
    */
    ...
    -- 保留、未対応、アプローチ中、日程調整中、面談済（電話）、電話済（来社）、
    -- 面談済（WEB）、面談済(LINE、メール、SMS) 、面談実施予定、MCR_アプローチ中、
    -- MCR_再アプローチ中、MCR_コンタクト済、MCR_送客済
    and crBase.cnslstatus_id in(
        1,4,6,30,31,32,34,37,38,39,40,43,44
    )
    /*
    1	未対応
    4	アプローチ中
    ...
    */
    ...
    -- ⑩登録ルート：マイナビ保育士、 マイナビ保育士(媒体)登録済 、保育士求人ナビ、保育士のミカタ 、ホイクトーク、保育士求人ぽけっと、ホイクナビ、リスジョブ(保育)
    and crBase.media_id in(34,37,38,127,128,155,157,163)
    /*
    34	ホイクナビ
    37	保育士求人ナビ
    38	保育士求人ぽけっと
    ...
    */
    ...
    --流入経路：保育士(105)/紹介(3)//空欄
    and (
        crBase.routeapply_id in (105,3,0)
        or crBase.routeapply_id is null
        )

    --稼働サービス領域 ：保育/その他/空欄
    and (
        crBase.active_service_field_id in (5,9,0)
...
WHERE
    -- 「正社員（正職員）」が「含まれていない
    crDsrdWkng.dsrdwayofwkngid not like '%/14/%'
